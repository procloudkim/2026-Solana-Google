...
- 결정: **사람은 각 구매자의 조건을 거래 전에 확인하고, 이후 선택·정책·거래 검증은 코드가 수행한다.**
...
현재 실행 상태와 남은 증거는 [환경·실행 런북](hackathon-environment-codex-runbook.md)이 단일 source of truth다.
...
| Test | 정책 mutation, idempotency, 실제 Solana Kit 메시지 생성·검증, build/test 수행 | 총 1 Devnet 테스트 USDC의 최종 공개 receipt와 실제 사용자 행동 검증 |

이 표는 디자인씽킹을 “완료했다”는 선언이 아니다. 현재 무엇을 배웠고 무엇이 비어 있는지 독자가 구분하도록 만든 증거 경계다.

## HITL의 정확한 위치

이 제품에서 HITL은 Agent가 고른 거래를 매번 승인하는 버튼이 아니다. 각 역할의 자연어 조건이 어떤 canonical mandate로 바뀌었는지 사람이 먼저 확인하는 단계다. 세 승인이 모두 있어야 Agent의 선택을 실행할 수 있다.

```mermaid
flowchart TD
    A["A/B/C가 자연어 조건 제시"] --> G["Gemini/ADK가 typed mandate 제안"]
    G --> H{"데모 운영자가 A/B/C 역할별<br/>mandate hash를 각각 확인했나?"}
    H -->|아니오| X["거래 생성 금지"]
    H -->|예| S["Agent가 공통 상품 또는 NO_BUY 제안"]
    S --> P{"결정론적 정책 재검사"]
    P -->|조건 불일치·만료| X
...
| 모드 | 누가 결정하는가 | 제품 행동 |
|---|---|---|
| `H-PRE` 조건 확인 | 데모 운영자가 A/B/C 역할별로 수행 | 각 canonical mandate hash와 일회성 nonce를 확인한다. 세 건이 모두 있어야 진행한다. |
| `A-PLAN` 후보 제안 | Gemini/ADK Agent | 자연어를 구조화하고 catalog에서 후보 또는 `NO_BUY`를 제안한다. 서명 권한은 없다. |
...
| `NO-BUY-CAP` | B cap 0.3 | 거래 0건, `NO_BUY` 또는 정책 거부 | 한 명의 조건도 희생하지 않음 |
...
1. [제품 README](../../product/mandate-pool/README.md)의 로컬 검증 명령으로 typecheck·test·build를 통과한다.
2. private fixture에서 정상·거부·승인 누락 경로를 실행하되 `NOT ON-CHAIN`을 유지한다.
3. private live revision의 read-only readiness와 지갑·ATA·mint를 확인한다.
4. 사람이 A/B/C mandate를 모두 확인한 뒤에만 총 1 Devnet 테스트 USDC 정상 경로를 실행한다.
5. 같은 revision에서 B cap 0.3 거부 경로가 transaction을 만들지 않았음을 보존한다.
6. 공개주소·transaction signature만 문서화하고 개인키·secret·`.env`는 저장소와 로그에 남기지 않는다.

현재 체크박스와 정확한 명령은 [실행 런북](hackathon-environment-codex-runbook.md)을 우선한다.
...
- [검증 메모](agentic-commerce-hitl-design-validation.md): 이 결정의 검증 범위와 남은 gate.
